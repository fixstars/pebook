...
TARGET_SCRIPT = os.path.join(PROJECT_ROOT, "src/tune/train_template.sh")

NUM_GPUS=16
PARAMETER_DISTRIBUTIONS = {
    "micro_batch_size": optuna.distributions.IntDistribution(1, 8),
    ...
    "context_parallel_size": optuna.distributions.IntDistribution(1, 8),
...
def get_cp_candidates(tensor_model_parallel_size, pipline_model_parallel_size):
    ...
    return [
        ...
        if PARAMETER_DISTRIBUTIONS["context_parallel_size"].low <= d <= PARAMETER_DISTRIBUTIONS["context_parallel_size"].high
    ]
...
def get_recompute_num_layers_candidates(recompute_granularity, pipeline_model_parallel_size):
    layer_per_stage = PARAMETER_DISTRIBUTIONS["recompute_num_layers"].high // pipeline_model_parallel_size
    return [0] if recompute_granularity != "full" else [
...
def command_generator(
    ...
    **kwargs,
    ...
    cp_index = trial.suggest_int("context_parallel_size_index", 0, len(cp_candidates) - 1)
    context_parallel_size = cp_candidates[cp_index]
    trial.set_user_attr("context_parallel_size", context_parallel_size)
    ...
    fp = trial.suggest_categorical("fp", PARAMETER_DISTRIBUTIONS["fp"].choices)
    recompute_granularity = trial.suggest_categorical("recompute_granularity", PARAMETER_DISTRIBUTIONS["recompute_granularity"].choices)

    recompute_method = trial.suggest_categorical("recompute_method", PARAMETER_DISTRIBUTIONS["recompute_method"].choices)
    if recompute_granularity == "selective" and recompute_method != "None":
        print("AssertionError: recompute method is not yet supported for selective recomputing granularity")
        return "exit 1"
    if recompute_granularity == "full" and recompute_method == "None":
        print('ValueError: Using recompute_granularity: full so recompute_method must be "block" or "uniform"')
        return "exit 1"

    recompute_num_layers_candidates = get_recompute_num_layers_candidates(recompute_granularity, pipeline_model_parallel_size)
...
def value_extractor(log_path: str) -> Optional[float]:
    ...
    for line in lines:
        if "throughput per GPU" in line:
            ...
            # 5iterのうち最初のサンプルを除くiterのTFLOP/s/GPUを抽出
            if 2 <= iteration and iteration <= 5:
                sample_flops.append(tflops_per_gpu)
    # 指定のiter数に満たなければ、タイムアウトなどのエラーが発生したと見なしNoneをreturn
    if len(sample_flops) < 4:
        return None
...
def main():
    ...
    tuner.optimize(command_generator, value_extractor, args.n_trials,
        default_params={
            'tensor_model_parallel_size_index': 3,
            'pipeline_model_parallel_size_index': 1,
            'context_parallel_size_index': 0,
            'micro_batch_size_index': 0,
            'recompute_num_layers_index': 1,
