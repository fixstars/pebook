...
def train():
    ...
    class NsysProfilerCallback(TrainerCallback):
        def on_step_begin(self, args, state, control, **kwargs):
            ...
            if state.global_step == 50:
                ...
                profiler.start()
            self.rng = nvtx.start_range(
                message=f"global_step:{state.global_step}", color="blue"
            )
    ...
    with torch.profiler.profile(
        schedule=torch.profiler.schedule(wait=40, warmup=3, active=2, repeat=1),
        ...
        if os.environ.get("PROFILE_NSYS") == "1":
            callbacks.append(NsysProfilerCallback)
        if os.environ.get("PROFILE_TORCH") == "1":
            callbacks.append(TorchProfilerCallback(prof))
